...
class BaseRepository:
    ...
    def leftJoin(self, table, first, operator, second):
        """Atalho para iniciar uma query com left_join"""
        return self.query().left_join(table, first, operator, second)
    ...
    def _prepare_for_update(self, data):
        ...
        print(f"_prepare_for_update chamado com data={data}")
        # Atualizar a variável brasil_now para obter a hora atual
        utc_now = datetime.now(pytz.UTC)
        current_brasil_now = utc_now.astimezone(BRASIL_TIMEZONE)
        data['updated_at'] = current_brasil_now
        print(f"_prepare_for_update retornando data={data}")
        return data
    ...
    def update(self, id, data):
        ...
        try:
            print(f"BaseRepository.update chamado com id={id}, data={data}")
            data = self._prepare_for_update(data)
            print(f"Dados após _prepare_for_update: {data}")
            ...
            print(f"Exceção ao atualizar registro: {e}")
            # Não propagar a exceção para evitar que a thread seja interrompida
            return False
